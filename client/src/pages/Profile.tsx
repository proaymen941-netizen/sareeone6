...
export default function Profile() {
  ...
  const menuItems = [
    ...
    { icon: Truck, label: language === 'ar' ? 'تطبيق الدلفري' : 'Driver App', path: '/driver', description: language === 'ar' ? 'انتقال إلى تطبيق السائقين' : 'Go to Driver App portal', testId: 'profile-delivery-app', onClick: () => { window.location.href = '/driver'; } },
    ...
    ...(shareUrl ? [{
      ...
      onClick: () => {
        if (navigator.share) {
          ...
          toast({ title: language === 'ar' ? 'نسخ الرابط' : 'Copy Link', description: shareUrl });
        }
      }
    }] : []),
