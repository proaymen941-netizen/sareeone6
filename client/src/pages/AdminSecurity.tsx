  ...
  const regenerateKeyMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      toast({
        ...
        description: `تم إعادة إنشاء مفتاح API لتطبيق ${data.target === 'driver' ? 'السائق' : 'العميل'} بنجاح`,
      });
    },
    onError: () => {
      toast({
        title: "خطأ",
        description: "حدث خطأ أثناء إعادة إنشاء مفتاح API",
        variant: "destructive",
      });
    }
  });
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="shadow-md border-blue-200 bg-gradient-to-br from-white via-blue-50/20 to-indigo-50/30 overflow-hidden">
            ...
            <CardContent className="p-5 space-y-6">
              ...
              {/* دليل برمجة تطبيق الأندرويد WebView Integration */}
              <div className="bg-slate-900 text-slate-100 p-4 rounded-xl border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Code2 className="w-4 h-4 text-orange-400" />
                    <h4 className="font-bold text-xs text-white">دليل برمجة تطبيق الأندرويد (Kotlin / Java Interface)</h4>
                  </div>
                ...
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  لبرمجة تطبيق الأندرويد للعميل أو السائق، قم بإنشاء واجهة <code className="text-orange-300 font-mono">AndroidBridge</code> وتمريرها إلى الـ WebView، أو قم بتمرير المفتاح في الرابط مباشرة:
                </p>

                <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 font-mono text-[11px] text-slate-200 dir-ltr text-left overflow-x-auto space-y-1">
                  <p className="text-slate-500">// 1. WebView Settings & JavaScript Interface</p>
                  <p className="text-blue-400">class <span className="text-amber-300">AndroidBridge</span>(private val context: Context) &#123;</p>
                  <p className="text-emerald-400">&nbsp;&nbsp;@JavascriptInterface</p>
                  <p>&nbsp;&nbsp;fun <span className="text-yellow-300">getApiKey</span>(): String = <span className="text-green-300">"{securitySettings?.customerApiKey || 'sareeone_cust_app_key_default'}"</span></p>
                  <p className="text-emerald-400">&nbsp;&nbsp;@JavascriptInterface</p>
                  <p>&nbsp;&nbsp;fun <span className="text-yellow-300">showNotification</span>(title: String, message: String) &#123;</p>
                  <p className="text-slate-400">&nbsp;&nbsp;&nbsp;&nbsp;// إظهار إشعار محلي فوري في شريط إشعارات أندرويد بدون أي سيرفر خارجي</p>
                  <p>&nbsp;&nbsp;&#125;</p>
                  <p className="text-blue-400">&#125;</p>
                  <p className="text-slate-500 pt-1">// 2. ربط الجسر بالـ WebView</p>
                  <p>webView.settings.javaScriptEnabled = <span className="text-orange-400">true</span></p>
                  <p>webView.settings.domStorageEnabled = <span className="text-orange-400">true</span></p>
                  <p>webView.settings.setGeolocationEnabled(<span className="text-orange-400">true</span>)</p>
                  <p>webView.addJavascriptInterface(AndroidBridge(this), <span className="text-green-300">"AndroidBridge"</span>)</p>
                </div>
              </div>
