...
import { Bike, MapPin, Clock, Phone, User, Eye, CheckCircle, XCircle, Truck, Search, UserCheck, Plus, Navigation, Loader2, FileText, Bell } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                    ...
                    توجيه الطلب للمندوبين (البث الفوري)
                  </h3>
                  {!selectedRequest.driverId && (
                    <Badge className="bg-amber-500 text-white text-[10px] px-2.5 py-0.5 rounded-full font-black animate-pulse">
                ...
                {!selectedRequest.driverId && (
                  <div className="p-3.5 bg-blue-50 border border-blue-200 rounded-[1.5rem] text-xs text-blue-900 space-y-1">
                    <div className="font-extrabold text-blue-950 flex items-center gap-1.5">
                      <Bell className="h-4 w-4 text-blue-600 animate-bounce" />
                      الطلب معروض حالياً في جميع هواتف السائقين:
                    </div>
                    <p className="text-blue-800 leading-relaxed text-[11px]">
                      تم بث طلب "وصل لي" لجميع السائقين المتاحين online مع تفعيل النغمة. أول سائق يضغط "قبول الطلب" يتسلم المهمة تلقائياً دون الحاجة لتدخلك.
                    </p>
                  </div>
                )}
