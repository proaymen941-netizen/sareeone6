...
export default function SpecialOffers() {
  ...
  const handleSave = async () => {
    try {
      ...
      const url = isEditing ? `/api/admin/special-offers/${editingId}` : '/api/admin/special-offers';
  ...
  const getOfferStatus = (offer: SpecialOffer) => {
    const now = new Date();
    const startDate = (offer as any).startDate ? new Date((offer as any).startDate) : new Date(offer.createdAt);
    const endDate = (offer as any).endDate ? new Date((offer as any).endDate) : (offer.validUntil ? new Date(offer.validUntil) : new Date(Date.now() + 8640000000));
