...
export const DriverDashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const acceptOrder = async (orderId: string) => {
    ...
    try {
      // السائق يقبل ويستلم الطلب مباشرة
      const response = await fetchWithAuth(`/api/drivers/orders/${orderId}/accept`, {
        method: 'POST',
      });
        ...
        {STEPS.map((step) => {
          ...
          return (
            ...
            >
              ...
              {step.id === 'available' && availableOrders.filter((o: any) => !['delivered', 'cancelled', 'completed'].includes(o.status)).length > 0 && (
                <span className="bg-red-500 text-white text-xs rounded-full px-2 py-0.5 font-black">
                  {availableOrders.filter((o: any) => !['delivered', 'cancelled', 'completed'].includes(o.status)).length}
                </span>
              )}
...
function AvailableOrdersSection({ orders, acceptingOrderId, onAccept, driver }: any) {
  // عرض كافة الطلبات المتاحة للتوصيل تلقائياً
  const filteredOrders = orders.filter((o: Order) => !['delivered', 'cancelled', 'completed'].includes(o.status));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
        <h2 className="text-xl font-black text-gray-900 flex items-center gap-2">
          ...
          الطلبات المتاحة للاستلام والتوصيل
        </h2>
