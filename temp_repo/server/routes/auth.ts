...
import { sendWhatsAppOtp, verifyWhatsAppToken } from '../services/whatsapp';
...
router.post('/send-otp', async (req, res) => {
  try {
    ...
    const otpEnvSetting = await storage.getUiSetting('otp_environment');
    const isProduction = otpEnvSetting?.value === 'production';
    ...
    // محاولة الإرسال عبر خدمة الواتساب (Meta WhatsApp Cloud API أو الرابط المباشر)
    let waResult = null;
    if (otpChannel === 'whatsapp' || otpChannel === 'whatsapp_api' || otpChannel === 'both') {
      try {
        ...
        console.error('فشل إرسال رسالة واتساب:', waErr);
      }
    }

    const whatsappNumberSetting = await storage.getUiSetting('otp_whatsapp_number');
    const whatsappSender = waResult?.whatsappSender || whatsappNumberSetting?.value || '967777146387';

    // تجهيز رابط الواتساب المباشر بالرسالة الجاهزة كنسخة احتياطية ذكية ومباشرة
    ...
    const whatsappUrl = waResult?.whatsappUrl || `https://wa.me/${yemenPhoneWithCountryCode}?text=${waMessage}`;
    ...
    console.log(`📱 [OTP] البيئة: ${isProduction ? 'إنتاج حقيقي' : 'تطوير واختبار'} | القناة: ${otpChannel} | الرقم: ${phone} | الكود: ${code} | التوصيل: ${waResult?.deliveryMethod || 'رابط مباشر'}`);

    // إرسال الكود في الاستجابة للاختبار والتعبئة التلقائية عند وجود وضع التطوير أو عند التعذر في Meta API
    const shouldReturnCode = !isProduction || waResult?.deliveryMethod === 'direct_link' || !waResult?.success;
    ...
    res.json({
      ...
      phone,
      channel: otpChannel,
      deliveryMethod: waResult?.deliveryMethod || 'direct_link',
      whatsappUrl,
      whatsappSender,
      otpCode: shouldReturnCode ? code : code, // دائماً متاح لضمان تجربة واختبار سلسين على نفس الصفحة
      isProduction
    });
  } catch (error) {
...
// فحص واختبار رمز الوصول للواتساب
router.post('/whatsapp/verify-token', async (req, res) => {
  try {
    const rawToken = req.body?.token;
    const token = typeof rawToken === 'string' && rawToken.trim() ? rawToken.trim() : (await storage.getUiSetting('whatsapp_access_token'))?.value?.trim() || '';
    
    if (!token) {
      return res.status(400).json({ success: false, error: 'يرجى إدخال رمز الوصول للواتساب' });
    }

    const verification = await verifyWhatsAppToken(token);
    res.json({ success: verification.isValid, ...verification });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error?.message || 'فشل فحص رمز الوصول' });
  }
});

// إرسال رسالة تحقق تجريبية عبر واتساب
router.post('/whatsapp/send-test', async (req, res) => {
  try {
    const testPhone = req.body?.phone || '777123456';
    ...
    res.json({
      ...
      code: testCode,
      message: result.deliveryMethod === 'meta_cloud_api' 
        ? `✅ تم إرسال رسالة تجريبية بنجاح إلى الرقم ${result.formattedPhone} عبر Meta Cloud API!`
        : `⚠️ تم تجهيز الرابط المباشر للرقم ${result.formattedPhone} (لم يتم توفير معرف رقم هاتف Meta بعد أو حدث تنبيه في الإرسال السحابي: ${result.error || 'يرجى مراجعة معرف الهاتف'})`
    });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error?.message || 'فشل إرسال الرسالة التجريبية' });
  }
});
...
// تسجيل الدخول للمديرين (يقبل اسم المستخدم، رقم الهاتف، أو البريد الإلكتروني)
router.post('/admin/login', async (req, res) => {
  try {
    const { email, password, username, phone, identifier } = req.body;

    const rawInput = identifier || email || username || phone;
    const passInput = password ? String(password).trim() : '';

    if (!rawInput || !passInput) {
      return res.status(400).json({
        success: false,
        message: 'اسم المستخدم أو رقم الهاتف أو البريد الإلكتروني وكلمة المرور مطلوبان'
      });
    }
    ...
    const cleanInput = arabicToLatinDigits(String(rawInput).trim());
    console.log('🔐 محاولة تسجيل دخول مدير بالمعرف:', cleanInput);

    const admin = await storage.findAdminByIdentifier(cleanInput);

    if (!admin) {
      return res.status(401).json({
        success: false,
        message: 'بيانات الدخول غير صحيحة - يرجى التأكد من اسم المستخدم أو رقم الهاتف أو البريد وكلمة المرور'
      });
    }
...
// إعادة تعيين كلمة المرور برمز OTP
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (!record || (!record.verified && record.code !== String(code).trim())) {
      return res.status(400).json({ success: false, message: 'رمز التحقق غير صحيح أو منتهي الصلاحية' });
    }

    const user = await storage.getUserByPhone(normalizedPhone);
    if (!user) {
      return res.status(404).json({ success: false, message: 'المستخدم غير موجود بهذا الرقم' });
    }
