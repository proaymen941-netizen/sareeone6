...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = async (item: MenuItem, restaurantId: string, restaurantName: string) => {
    // فحص إذا كان المستخدم زارياً
    const isGuest = localStorage.getItem('is_guest') === 'true';
    const authToken = localStorage.getItem('auth_token');
    if (isGuest && !authToken) {
      toast({
        title: "تسجيل الحساب مطلوب",
        description: "عذراً، لا يمكنك إضافة منتجات للسلة أو إتمام الطلب وأنت تصفح كزائر. يرجى تسجيل حساب أو تسجيل الدخول للمواصلة.",
        variant: "destructive",
      });
      setTimeout(() => {
