...
export async function ensureAdminUsers() {
  try {
    const email = "aimenmo123@gmail.com";
    const phone = "777146387";
    const username = "aimenmo";
    const password = "1234567";

    const existing = (await storage.getAdminByEmail(email)) || 
                     (await storage.getAdminByEmail(username)) || 
                     (await storage.getAdminByPhone(phone));
    if (!existing) {
      ...
      console.log('👤 تم إنشاء حساب مدير النظام بنجاح: ايمن محمد');
    } else {
      ...
      console.log('👤 تم تحديث حساب مدير النظام بنجاح وتعيين كلمة المرور: ايمن محمد');
    }
  } catch (error) {
    console.error('خطأ في ضمان حساب المدير:', error);
  }
}
